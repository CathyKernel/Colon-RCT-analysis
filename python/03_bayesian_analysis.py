...
axes[0].set_xlim(0.15, 1.05)

# Right panel: Bayesian monitoring statistic P(HR<1 | data) at each look.
# The four posterior probabilities are all very close to 1 (0.9985, 0.998,
# 1.000, 1.000) because even at 25% of information the posterior HR (0.60,
# 95% CrI 0.40-0.88) is already entirely below 1. On a [0, 1] axis the line
# therefore looks flat near the top -- which is the truthful scientific
# message ("benefit already established at the first interim look"). To make
# the actual numbers readable we (a) annotate each point with its value, and
# (b) add a small subtitle explaining the flatness.
axes[1].plot(seq_tab["info_fraction"], seq_tab["P_HR_lt_1"], "o-",
             color=common.BLUE, markersize=6, linewidth=1.5, zorder=3)
axes[1].axhline(0.975, color=common.ACCENT, linestyle="--", linewidth=0.9,
                zorder=2)
# Move the threshold label to the LEFT of the first data point (x=0.25) and
# place it just under the dashed line, so it no longer overlaps the data
# line/points.
axes[1].text(0.18, 0.965, "0.975 threshold",
             color=common.ACCENT, fontsize=9, ha="left", va="top",
             bbox=dict(facecolor="white", alpha=0.9, edgecolor="none",
                       pad=1.5))
# Value labels next to each point -- offset ABOVE each point so they don't
# collide with the connecting line (which runs beneath the points at the
# top of the panel).
for _, r in seq_tab.iterrows():
    p = r["P_HR_lt_1"]
    label = "1.000" if p >= 0.9995 else f"{p:.3f}"
    axes[1].annotate(label,
                     xy=(r["info_fraction"], p),
                     xytext=(0, 8), textcoords="offset points",
                     fontsize=8.5, color=common.BLUE,
                     ha="center", va="bottom")
...
# Zoom the y-axis to [0.9, 1.04] so the small variations between looks are
# visible rather than collapsed onto a single line, and so the value labels
# above each point have room to breathe.
axes[1].set_ylim(0.9, 1.04)
axes[1].set_xlim(0.15, 1.05)
axes[1].text(0.17, 0.905, "axis broken",
             fontsize=7.5, color="grey", ha="left", va="bottom", style="italic")
# Explanatory subtitle: tells the reader why the line is flat.
axes[1].text(0.5, -0.30,
             "Benefit (P>0.975) signalled at the first interim look (25% of\n"
             "information); posterior remains decisive at all later looks.",
             transform=axes[1].transAxes, fontsize=8, color="#444444",
