...
axes[0].set_xlabel("Harrell's C-index (25 CV folds)")
axes[0].set_title("Cross-validated C-index")
...
# Horizontal AUC panel aligned with the left panel's model rows: the previous
# vertical layout printed the long model names as x tick labels, which
# collided with each other and with the neighbouring y-axis tick labels.
pos = {m: i + 1 for i, m in enumerate(order)}
yp = [pos[m] for m in means.index]
axes[1].errorbar(means.values, yp, xerr=sds.values, fmt="o",
                 color=common.ACCENT, capsize=3, markersize=5)
axes[1].set_xlim(0.5, 1.0)
axes[1].set_ylim(0.5, len(order) + 0.5)
axes[1].set_yticks(range(1, len(order) + 1))
axes[1].set_yticklabels([])
axes[1].set_xlabel(f"AUC at t = {HORIZON:.0f} years (IPCW)")
