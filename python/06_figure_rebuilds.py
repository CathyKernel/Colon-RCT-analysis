...
def km_figure(df, tvar, svar, endpoint, title, subtitle, name):
    ...
    # height_ratios: the risk-table panel was 1.6 (too tall -> rows far apart);
    # tightened to 1.0 so the three at-risk rows sit at a natural text-leading
    # distance. Overall figure height reduced from 5.8in to 5.2in to keep the
    # KM panel from being squashed by the relatively smaller table.
    fig, (ax, axr) = plt.subplots(
        2, 1, figsize=(7.0, 5.2), sharex=True,
        gridspec_kw={"height_ratios": [3.4, 1.0]}, layout="constrained")
    ...
    # Tighten the row spacing: rows are 0.7 units apart (was 1.0) so the
    # three at-risk rows form a compact table rather than three loose rows.
    row_step = 0.7
    for i, arm in enumerate(ARM_LEVELS):
        sub = df[df["rx"] == arm]
        y = (n_arms - 1 - i) * row_step        # Obs on top, Lev+5FU bottom
        vals = [n_at_risk(sub[tvar].values, tt) for tt in times]
        for tt, v in zip(times, vals):
            axr.text(tt, y, str(v), ha="center", va="center",
                     fontsize=10.5, color=ARM_COLS[arm])
    axr.set_yticks([(n_arms - 1 - i) * row_step for i in range(n_arms)])
    axr.set_yticklabels([ARM_LABELS[a] for a in ARM_LEVELS], fontsize=10)
    axr.tick_params(axis="y", pad=3)
    for lab, arm in zip(axr.get_yticklabels(), ARM_LEVELS):
        lab.set_color(ARM_COLS[arm])
    # Tighter ylim so rows don't swim in whitespace.
    axr.set_ylim(-0.45, (n_arms - 1) * row_step + 0.45)
    axr.set_xlim(-0.6, max_t)
    axr.set_xticks(times)
    axr.tick_params(axis="x", labelsize=10.5, pad=2)
    # Pull the "Years since randomization" xlabel closer to the tick labels
    # (default labelpad is ~6pt which left a visible gap).
    axr.set_xlabel("Years since randomization", fontsize=11, labelpad=2)
    axr.set_title("Number at risk", loc="left", fontsize=10.5,
...
def sim_efficiency():
    ...
    x = np.arange(len(tab))
    w = 0.35
    # Slightly taller figure (was 4.1in) so the upper-left quadrant (where
    # the subtitle, legend, and bar value labels all live) has room to
    # breathe instead of being vertically compressed.
    ...
    ax.bar(x + w / 2, tab["power_ad"], w, label="Adjusted",
           color=common.GREEN)
    # Numeric value labels above each bar: pushed further up (offset 0.030
    # was 0.015) so they no longer crowd the bar tops.
    for xi, (pu, pa) in zip(x, zip(tab["power_un"], tab["power_ad"])):
        ax.text(xi - w / 2, pu + 0.030, f"{pu:.3f}", ha="center",
                va="bottom", fontsize=9, color=common.BLUE)
        ax.text(xi + w / 2, pa + 0.030, f"{pa:.3f}", ha="center",
                va="bottom", fontsize=9, color=common.GREEN)
    ax.axhline(0.8, color="grey", linestyle="--", linewidth=0.9)
    # "80% target" annotation: move it to the RIGHT edge so it no longer
    # sits in the crowded upper-left quadrant (where it overlapped the
    # legend and bar value labels).
    ax.text(len(tab) - 0.55, 0.812, "80% target", fontsize=9, color="grey",
            ha="right", va="bottom")
    ax.set_xticks(x)
    ax.set_xticklabels(tab["strength_lab"], fontsize=10.5)
    # Extend y-axis upper limit to 1.08 (was 1.0) so the raised value
    # labels and the legend have headroom.
    ax.set_ylim(0, 1.08)
    ax.yaxis.set_major_formatter(mtick.PercentFormatter(1.0, decimals=0))
    ...
    # Add explicit `pad` to the subtitle so it sits further below the bar
    # value labels instead of crowding them.
    ax.set_title("1,000 replicates per scenario, n = 322, target HR = 0.72, "
                 ...
                 fontsize=9.5, color=GREY_TEXT, pad=14)
    # Legend moved to upper RIGHT (was upper left) so it does not collide
    # with the bar value labels of the leftmost scenario group.
