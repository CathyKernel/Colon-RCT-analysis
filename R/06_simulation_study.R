...
figA <- resA %>%
  ...
  labs(title = "Simulation Study A: power gain from covariate adjustment",
       subtitle = sprintf("%d replicates per scenario, n = %d, target HR = %.2f, one-sided alpha = 0.025\nlog-rank test (not shown) is numerically equivalent to the unadjusted Cox test",
                          NSIM_A, N_TRIAL, HR_A),
